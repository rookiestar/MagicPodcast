...
### 5.4. Context Window 管理策略
为避免 "The model has reached its context window limit" 错误：

**对话会话管理**：
- ✅ **每次会话只完成 1-2 个相关功能**
- ✅ **完成功能后立即提交到 git**
- ✅ **提交后建议用户结束当前会话，开始新会话**
- ✅ **使用简洁的提示词启动新会话**（例如："继续实现下一个功能切片：工作流报告生成"）

**推荐的会话周期**：
```
功能开发 → 测试 → git commit → 结束会话 → 新会话继续
```

**提示词优化**：
- 新会话开始时，避免重新加载大量上下文
- 使用 git log 和 git diff 让 AI 快速了解最近变更
- 遵循 CLAUDE.md 中定义的架构，无需重复解释

**特殊情况处理**：
- 如果必须处理多个功能，使用 Task 工具启动子进程
- 对于探索性任务，使用 Explore agent 保持主会话精简
- 定期清理不必要的文件（如 .bak、-old.tsx 等）
