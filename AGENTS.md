...
5. **最小充分防御**：默认复用现有行为/API 契约、Fixture/Snapshot、测试和门禁。只有先明确可复现的具体失败场景或可信边界要求，并证明现有机制无法阻止或识别该失败时，才新增直接覆盖缺口的最小 hash/指纹、契约冻结副本、baseline 或 gate；可由指令/人审覆盖的判断留在审查层，不机械化为 validator、别名/错误码表或规则引擎。同一信任边界内同一事实默认只校验一次并保持单一账本。新增前须记录失败场景、已检查机制、机制缺口，以及新增机制为何最小且充分；任务内错误处理的严格性只约束真实错误路径，不单独授权增设检查或兼容层。
6. **Verify**：按 §5 验证；排障先查用户点名系统，仅在证据指向相邻工具时延伸。一次只做一个最小变更，复验原症状，通过即停。
