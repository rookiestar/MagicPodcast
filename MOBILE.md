...
日常移动设备访问只能使用生产 HTTPS 域名，并通过 Cloudflare Access 的 Google 登录。前端和后端不得为手机、平板或局域网设备开放直连端口。
...
2. 仅使用所有者指定、已启用 MFA 或 Passkey 的 Google 身份登录 Cloudflare Access。
...
- Cloudflare Access 是否允许当前 Google 身份。
