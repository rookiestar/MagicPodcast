...
> 说明：本文保留 2026-02-01 的早期基线，用于历史对比。最新可复跑基线见 [../../performance/BASELINE_2026-05-31.md](../../performance/BASELINE_2026-05-31.md)。
...
| 后端测试 | `go test ./...` 通过 |
| 后端静态检查 | `go vet ./...` 通过 |
| 搜索服务并发检查 | `go test -race ./internal/services` 通过 |
| 前端类型检查 | `npm run type-check` 通过 |
| 前端 Lint | `npm run lint` 通过 |
| 前端测试 | 56 个测试文件、296 个用例通过 |
| 前端生产构建 | `npm run build` 通过 |
| 页面巡检 | 7 个核心页面全部 OK |
| API 巡检 | 11 个接口全部 OK |
| 服务重启健康检查 | 后端数据库正常，前端 HTTP 200 |
| 搜索服务微基准 | 已可在内存数据集上独立运行 |

当前主要性能瓶颈仍是搜索：轻量巡检中默认预热后的搜索 P95 为 127ms、平均为 126.9ms，并发基准下全文搜索 P95 为 187ms，已经低于 200ms 目标线；但重启后第一跳仍可能超过 500ms。后续优化优先围绕搜索冷启动、查询路径和缓存策略展开。
