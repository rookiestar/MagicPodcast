...
以下检查必须有覆盖发布候选的成功证据；不要求在本地与 CI 重复执行。固定 main SHA 的成功 CI 可覆盖其中实际运行的 Go、前端测试、类型检查、Lint 与构建；本地按改动风险做定向检查和实际页面验收。候选或相关环境变化后仅复用仍有效的结果，失败结果不能由其他定向检查覆盖。若本地或目标平台已有未解释的失败，即使 Linux CI 成功也不能直接判为通过；先定位差异，修复后重跑受影响检查。Linux 构建证据不替代发布脚本在生产目标平台的构建、产物配对和激活验证。
