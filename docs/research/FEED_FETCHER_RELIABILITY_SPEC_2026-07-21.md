...
> 后续决策：#35/#36 已取代本文 P1 中“小宇宙首次 403 整域硬断路”的设计，并收紧 P2 的 last-good 语义；#44 将批次联网窗口调整为 10 分钟，访问拒绝重试节点调整为第 2/5/8 分钟。目标方案改为共享单队列、简单自适应软限速和该窗口内的有界分类重试；last-good 仅用于匹配验证器的 304 恢复和诊断，不计本批成功、不进入新报告、不推进抓取时间。本文已实现的 HTTP、持久化与观测能力继续有效。当前源码已完成 #36/#44 的上述批次策略；生产运行态仍需发布后单独验证。
...
- 本文撰写时，`feed.xyzfm.space` 仍会在一次 403 后 OPEN 10 分钟；#35/#36 随后删除了这项整域硬断路，#44 再将批次联网窗口与访问拒绝重试节点调整为 10 分钟和第 2/5/8 分钟。timeout、网络错误、500/502/504 也可能只影响同域某一个 Feed，不能不加区分地升级为域名故障。
...
- **P1 Feed Failure Recovery**：本文原有整域断路方案已被 #35/#36 取代；#44 将小宇宙策略收口为共享单队列、简单自适应软限速和 10 分钟批次内第 2/5/8 分钟的有界分类重试。
...
- **403 规则**：`feed.xyzfm.space` 的单个 403 不再触发整域 OPEN，按 #35/#36/#44 使用共享单队列、软限速和约第 2/5/8 分钟的有界重试；其他域名默认也不因单个 Feed 的一次 403 触发域名熔断，可按域显式配置。
...
- `feed.xyzfm.space` 单个或多个 403 不阻断其他同域 Feed，也不产生新的 `circuit_open`；10 分钟批次内按约第 2/5/8 分钟有界重试。
...
| **P1** | xyzfm 共享单队列 + 简单自适应软限速 + 10 分钟内第 2/5/8 分钟有界分类重试；其他域名保留多 Feed 域故障证据门槛 | P3a | 中（改抓取准入语义） | 是 |
| **P3b** | 轻量内存计数器 + 独立受保护 admin JSON + ConfiguredEgressLabel + 403 runbook | P3a | 中（新增受保护观测面） | 是 |

**顺序理由**：先做不依赖数据库的 P0a 与 P3a；再用 P2a 建立“正文+验证器”原子持久化，之后 P0b 才能安全处理 304。P1 按 #35/#36/#44 删除 xyzfm 整域硬断路并加入软限速与 10 分钟有界重试；P3b 在访问控制确认后上线。每阶段独立测试、提交、发布和回滚；数据库迁移与行为变化不混在同一发布。
...
1. **Feed 抓取参数配置化与默认值**：把冷却、retry budget、超时、UA 改为启动时配置；所有域名可采用保守负载整形。`feed.xyzfm.space` 按 #35/#36/#44 删除整域硬断路，改为共享单队列、简单自适应软限速和 10 分钟批次内第 2/5/8 分钟的有界分类重试。
