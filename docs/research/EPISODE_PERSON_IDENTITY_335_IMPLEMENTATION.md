...
- PR #340 已合入 `main`，合并提交为 `f8ea534`。合并后审查发现两处生命周期缺口，未改变人物识别合同或生产数据范围。
- `rebuild_episode_people` 现在以 `sqlite-logical-v1` 逻辑指纹绑定备份与预览目标库，覆盖 schema、索引/触发器/视图及用户表数据；写入前重新打开目标库再次核对，健康但无关的备份会拒绝。`quick_check` 仍保留，不能替代恢复演练。
- 同一单集没有跨集身份锚点的同名人物，现在会通过精确 Speaker、片段位置和原文引文绑定已有本地 ID；匹配不唯一时保持拆分，重复准备与排除后重建均保持 ID 稳定。
- 新增3项回归测试覆盖上述边界；完整门禁、follow-up PR 和 GitHub 合并状态以最终提交后的实时记录为准。生产迁移、回填、部署与关票不在本次范围。
