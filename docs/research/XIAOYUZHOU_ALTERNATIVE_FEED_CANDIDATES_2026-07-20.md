...
日期：2026-07-20
状态：候选研究与已验证接入；仅补充 2 条稳定身份，未修改主 Feed URL 或生产网络

> 后续决策：#35–#37 保留“已验证替代源、不永久修改主 Feed”的边界，但已取代本文的整域断路和普通失败后 last-good 兜底语义；#44 将批次联网窗口调整为 10 分钟，访问拒绝重试节点调整为第 2/5/8 分钟。目标方案是在该窗口内重试主源并立即尝试已验证替代源；last-good 只用于匹配验证器的 304 恢复和诊断。
...
其余 7 个节目目前没有在 PodcastIndex 中发现第二个非小宇宙 Feed，Apple 目录结果也仍指向原小宇宙 Feed。因此它们不因同名搜索结果自动切换；后续按 #35/#36 对主 Feed 做有界分类重试，不以普通失败后的 last-good 计作成功。

## 生产激活与验证

候选接入前先创建并验证了生产备份：

- 文件：`backend/data/backups/magicpodcast_20260720_211145.db.gz`
- SHA-256：`15192e2e82b2a20489ca42dff9a6781aacde1428aa296c09a228e7fcb4b76346`
- 备份校验：完整性、外键、487 个播客、64,740 个单集、51 个标签、18 个工作流均通过。

只在服务停止期间执行了两条带原 Feed URL 和空值条件的事务更新：

| 生产 ID | 写入字段 | 写入值 | 未改变 |
| ---: | --- | --- | --- |
| 85 | `i_tunes_id` | `1525369049` | `feed_url`、`podcast_guid` |
| 195 | `i_tunes_id` | `1612954022` | `feed_url`、`podcast_guid` |

7 个没有替代源的节目没有写入猜测身份。服务随后发布到 release `20260720T132843Z-72a30fa-30522`，`/health`、`/ready`、前端、SQLite 完整性、外键和异机加密备份均通过。

发布后只做了两个受控的单节目增量同步，没有运行全量工作流：

| 节目 | 主源结果 | 实际来源 | 身份结果 | 变化 |
| --- | --- | --- | --- | --- |
| 85 | 小宇宙 Feed 返回拒绝 | `http://www.ximalaya.com/album/40320716.xml` | `verified_stable_identity_and_metadata` | 新增 5 集 |
| 195 | 小宇宙域名已断路，未再次发起主源请求 | `http://www.ximalaya.com/album/21469108.xml` | `verified_stable_identity_and_metadata` | 新增 1 集 |

两个结果都记录为 `source_type=alternative`、`egress_id=direct`；没有代理、换 IP 或新增重试。日志还显示 85 的第一次主源拒绝后才触发替代源，195 没有第二次小宇宙 HTTP GET，说明域名断路仍在替代链前面生效。
...
这些结果能补充主 Feed 的稳定身份，但没有产生非小宇宙替代源；它们后续依赖 #35/#36 的软限速和有界分类重试，不能用整域断路或普通失败后的 last-good 代替实时抽取。

## 接入边界

1. 不修改 9 个节目的主 `feed_url`；替代源只在本批主 Feed 尝试失败后使用。
2. 只把有稳定 iTunes ID/GUID 且有标题、作者或单集证据的候选交给现有 PodcastIndex fallback。
3. 本批按主 Feed → 已验证替代 Feed 执行；last-good 只用于匹配验证器的 304 恢复和诊断，不计成功、不进入新报告、不推进抓取时间。
4. `feed.xyzfm.space` 按 #35/#36 使用共享单队列、软限速和有界分类重试；替代请求仍通过同一个 FeedFetcher 和现有观测链路。
5. 候选身份激活应先备份主库，只补 85 和 195 的稳定 ID，保留更新前值；7 个没有候选的节目不写入猜测值。
6. 如果候选 Feed 后续出现解析失败、身份冲突或内容证据不足，立即停止选择该候选并记录本批失败；不扩大候选池，也不以普通失败后的 last-good 伪装成功。

本次实际验证还发现一个需要后续单独处理的跨源去重问题：小宇宙与 Ximalaya 为同一 E150 生成了不同 GUID，85 的增量同步因此新增了 4 个新节目加 1 条 E150 重复记录。该记录未直接删除，避免在没有合并规则和用户确认时破坏真实数据；后续应为跨 Feed 切换增加保守的逻辑单集匹配，并单独验证回滚。
