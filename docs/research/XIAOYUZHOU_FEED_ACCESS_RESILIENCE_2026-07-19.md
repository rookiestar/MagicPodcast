...
> 后续决策：#35/#36 已取代本文关于“小宇宙首次 403 整域硬断路”的建议；#44 将批次联网窗口调整为 10 分钟，访问拒绝重试节点调整为第 2/5/8 分钟。目标方案改为共享单队列、简单自适应软限速和该窗口内的有界分类重试；本文其余一手证据和固定出口边界仍可参考。
...
4. `429/503` 遵守 `Retry-After`；小宇宙 403 按 #35/#36/#44 在 10 分钟批次内于约第 2/5/8 分钟有界重试，并通过共享单队列和简单自适应软限速降低压力，不再触发整域硬断路，也不自动换出口。其他域名的域名级熔断仍需显式策略或多个不同 Feed 的共同失败证据。
