...
- 页面资源同时报告 HTML 中确定引用的静态包编码传输量和源文件体积。严格门禁只按编码传输量判断；源文件体积保留为提示，不再把未压缩 `HEAD Content-Length` 当作用户下载量。
...
- HTTP 静态包统计只覆盖 HTML 明确引用的 `/_next/static/`、图标和 manifest；不会把响应式图片的 HTML 兜底尺寸当成浏览器实际下载。
- 浏览器缓存命中、动态加载资源、响应式图片和字体仍须用真实浏览器资源检查确认；HTTP 巡检不能代替冷载/热回访验收。
