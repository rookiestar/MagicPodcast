...
- 公共 Cloudflare 路径仍有跨境长尾，综合验收 Issue 保持开放。
...
| [#81](https://github.com/rookiestar/MagicPodcast/issues/81) | 滚动封面稳定性 | 开放 | 代码已交付，公网综合证据仍未收口 |
| [#82](https://github.com/rookiestar/MagicPodcast/issues/82) | 公网综合验收 | 开放 | 仍受 #81 阻塞；公共绝对时延未解决 |

公网长尾已由 [#64](https://github.com/rookiestar/MagicPodcast/issues/64) 跟踪和验证，本轮没有创建重复基础设施 Issue。

以上状态于 2026-08-09 从 GitHub 实时复核。
...
2026-08-09 复核时，生产 `/health` 仍返回上述 release/build，数据库状态正常。
...
| 验收面 | 结果 |
| --- | --- |
| 生产版本 | release `20260809T114543Z-2941a03-60126` 健康 |
| 私网页面 | `/podcasts` 200 |
| 私网 API | 返回 10 项 |
...
- #81 需要补齐约定的生产功能、缓存复用、失败率和预取行为证据后再关闭。
- #82 必须在 #81 关闭后重新做综合验收。
