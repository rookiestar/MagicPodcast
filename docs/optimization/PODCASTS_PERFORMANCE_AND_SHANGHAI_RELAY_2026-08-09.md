...
状态：应用优化已发布；上海私网入口已升级为正式主路径；Cloudflare 为备用路径

## 0. 2026-08-16 访问策略更新

所有者已确认将上海私网中继作为正式主路径，Cloudflare Access 作为公共备用路径。

- 主路径继续使用既有 loopback / WireGuard 私网，不新增公网 Web 暴露。
- Cloudflare、公共 DNS和 Access 保持不变，承担手动故障回退。
- 主路径继续执行冷载 20 次、成功率 100%、P95 `< 5s`、最大 `< 10s` 的硬门槛。
- 公共路径保留可用性与安全门禁，绝对时延单独记录，不再替代主路径验收。

当前权威操作入口见[正式访问路径](../runbooks/PRIMARY_ACCESS_PATH.md)。以下历史数据继续用于解释方案来源，不代表公共路径已经变快。
