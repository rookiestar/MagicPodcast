...
| 单集视频三态列（详情页「看视频」） | [research/PODCAST_DETAIL_VIDEO_LINK_SPEC_2026-08-27.md](research/PODCAST_DETAIL_VIDEO_LINK_SPEC_2026-08-27.md)（#199）：源码已登记 schema 23 `episodes.video_availability`；不存 HLS。属真实数据库结构变更 | 代码已落地；生产 `--apply` 仍须按迁移指南备份、停服务、确认字符串。未授权不得写真实库 |
