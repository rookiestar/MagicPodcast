...
| Feed 抓取参数配置化与默认值 | 详见 [research/FEED_FETCHER_RELIABILITY_SPEC_2026-07-21.md](research/FEED_FETCHER_RELIABILITY_SPEC_2026-07-21.md) P0/P1 及 #35/#36：冷却、retry budget、超时、User-Agent 为启动时配置；小宇宙为共享单队列 + 自适应软限速 + 15 分钟有界分类重试 | #36 已删除 xyzfm 首次 403 整域硬断路；生产侧仍需一次真实批次验收确认不再出现新的 `circuit_open`（沙箱以 httptest seam 证明）。其余 #35 子票（替代源预验证、finalizing、尝试链、补偿）独立跟踪 |
