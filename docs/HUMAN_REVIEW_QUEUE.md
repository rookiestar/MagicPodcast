...
| Feed 抓取参数配置化与默认值 | 详见 [research/FEED_FETCHER_RELIABILITY_SPEC_2026-07-21.md](research/FEED_FETCHER_RELIABILITY_SPEC_2026-07-21.md) P0/P1 及 #35/#36/#44：冷却、retry budget、超时、User-Agent 为启动时配置；小宇宙为共享单队列 + 自适应软限速 + 10 分钟有界分类重试（403 节点 2/5/8 分钟） | #36 已删除 xyzfm 首次 403 整域硬断路；#44 已将批次窗口与重试节点改为 10 分钟 / 2/5/8。生产侧仍需一次真实批次验收确认不再出现新的 `circuit_open`（沙箱以 httptest seam 证明）。其余 #35 子票（替代源预验证、finalizing、尝试链、补偿）独立跟踪 |
| 持久化 Feed 快照数据库迁移 | 同上 Spec P2：新增 `feed_snapshots` 表，将正文、指纹和验证器原子保存，用于匹配验证器的 304 恢复与诊断；不在 `podcasts` 复制第二份权威验证器；属真实数据库结构变更 | 仅按 `migration/MIGRATION_GUIDE.md` 先备份、验证、停服务，再用确认字符串经 `cmd/migrate` 运行；验收 256 条/32 MiB 上限和确定性淘汰；按 #35 不得把普通失败后的 last-good 计作本批成功或写入新报告 |
...
| 单集视频三态列（详情页「看视频」） | [research/PODCAST_DETAIL_VIDEO_LINK_SPEC_2026-08-27.md](research/PODCAST_DETAIL_VIDEO_LINK_SPEC_2026-08-27.md)（#199）：源码已登记 schema 24 `episodes.video_availability` 及其 CHECK 约束；不存 HLS。属真实数据库结构变更 | 代码已落地；生产 `--apply` 仍须按迁移指南备份、停服务、确认字符串。未授权不得写真实库 |
