...
- Go 与 Python 只通过版本化 `stdio` JSONL 通信，不使用 WebSocket；当前执行帧协议版本为 3。版本 3 在既有执行、事件、取消和结构化结果之外增加结构化 `progress` 帧：Host 把稳定的 SDK Turn、Item、网页搜索、可用推理摘要、Agent 文本与终态通知映射为供应商中立、脱敏、有预算的活动（稳定活动 ID、连续序号、类别、开始/更新/完成/失败状态、长度受限的纯文本、有界公开元数据）；原始 Prompt、思维链、私有备注、受管路径、凭据和未知供应商载荷永不进入 progress。模块对乱序、冲突、超限和终态后事件继续 fail-closed。
