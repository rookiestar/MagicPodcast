...
每次执行使用临时 Codex Home，只复用主机文件认证；Shell、文件写入、MCP、Plugin、Skill、图片和子代理等能力在模型获得工具目录前关闭，只按执行类型开放网页检索。SDK 与 CLI Runtime 必须同时匹配固定版本；进程内终态历史有界保留，长期权威状态仍在 MagicPodcast 数据库。
