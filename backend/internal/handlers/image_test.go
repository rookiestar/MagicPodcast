...
func TestImageHandler_AllowsReviewedPNGWithBoundedCaching(t *testing.T) {
	...
	// 受控缓存：回写有界 max-age，而非 no-store。
	assert.Equal(t, imageProxyCacheControl, recorder.Header().Get("Cache-Control"))
...
func TestImageHandler_RejectsRedirectsToDisallowedTargets(t *testing.T) {
	// 逐跳校验：重定向目标为私网/带端口/非白名单时，CheckRedirect 拒绝，Do() 失败 → 502。
	handler := NewImageHandler()
	handler.httpClient.Transport = imageRoundTripper(func(*http.Request) (*http.Response, error) {
...
func TestImageHandler_FollowsValidatedRedirectsToReviewedHosts(t *testing.T) {
	// 逐跳校验：重定向目标落在白名单内时允许跟随，最终返回 200。
	handler := NewImageHandler()
