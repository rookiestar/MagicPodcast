...
func (s *Service) syncPodcastMetadataWithUpdateCheck(podcast *models.Podcast, reporter ProgressReporter) (error, bool, *EpisodeSyncResult) {
	...
	case gofeed := <-resultChan:
		...
		// Debug日志
		if hasUpdate {
			logger.Infof("🔍 检测到更新: %s | 原因: %s", podcast.Title, strings.Join(updateReasons, ", "))
		}
		...
		if existingEpisodeCount == 0 {
			...
			logger.Infof("   [%s] 无单集，使用全量同步", podcast.Title)
		} else if hasUpdate {
			// 如果元数据有更新，使用全量模式（确保获取所有单集）
			episodeSyncMode = SyncModeFull
			logger.Infof("   [%s] 元数据有更新，使用全量同步", podcast.Title)
		} else if existingEpisodeCount != feedEpisodeCount {
			// 如果单集数量不匹配，使用全量模式同步
			episodeSyncMode = SyncModeFull
			logger.Infof("   [%s] 单集数量不匹配 (数据库:%d, feed:%d)，使用全量同步", podcast.Title, existingEpisodeCount, feedEpisodeCount)
		} else {
			// 元数据无更新且单集数量匹配，跳过单集同步
			logger.Infof("   [%s] 元数据无更新且单集数量匹配(%d)，跳过单集同步", podcast.Title, existingEpisodeCount)
			return nil, true, nil
		}
