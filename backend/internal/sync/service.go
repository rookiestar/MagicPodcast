...
// FullSyncConfig 完整同步配置
type FullSyncConfig struct {
	MetadataSyncMode string // 元数据同步模式: incremental, full
	EpisodeSyncMode  string // 单集同步模式: incremental, full, smart
}

// DefaultFullSyncConfig 默认完整同步配置
var DefaultFullSyncConfig = FullSyncConfig{
	MetadataSyncMode: "incremental",
	EpisodeSyncMode:  "full", // 完整同步使用full模式，获取所有单集
}

// SyncPodcastsFull 完整同步：元数据 + 单集
func (s *Service) SyncPodcastsFull(reporter ProgressReporter, config FullSyncConfig) error {
	startTime := time.Now()
	log.Println("🚀 开始完整同步（元数据 + 单集）...")
	reporter.Report("开始完整同步流程...")

	// 第一步：同步元数据
	reporter.Report("========== 第1步：同步播客元数据 ==========")
	if err := s.SyncPodcastsMetadataSSE(reporter); err != nil {
		reporter.ReportError(fmt.Sprintf("元数据同步失败: %v", err))
		return fmt.Errorf("元数据同步失败: %w", err)
	}

	// 第二步：同步单集
	reporter.Report("========== 第2步：同步单集信息 ==========")

	// 构建单集同步配置
	episodeConfig := DefaultEpisodeSyncConfig
	switch config.EpisodeSyncMode {
	case "incremental":
		episodeConfig.Mode = SyncModeIncremental
	case "full":
		episodeConfig.Mode = SyncModeFull
	case "smart":
		episodeConfig.Mode = SyncModeSmart
	default:
		episodeConfig.Mode = SyncModeSmart
	}

	if err := s.SyncAllPodcastEpisodes(reporter, episodeConfig); err != nil {
		reporter.ReportError(fmt.Sprintf("单集同步失败: %v", err))
		return fmt.Errorf("单集同步失败: %w", err)
	}
	...
	// 发送完整同步的汇总统计
	// 注意：这里我们简化处理，使用SyncAllPodcastEpisodes已经发送的summary
	// 但为了明确表示这是完整同步完成，我们再发送一次带有duration的summary
	reporter.ReportSuccess(fmt.Sprintf("✅ 完整同步完成！总耗时: %s", formatDuration(duration)))

	log.Printf("✅ 完整同步完成，总耗时: %s", formatDuration(duration))
	return nil
}
