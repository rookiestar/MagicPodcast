...
对每个没有候选的 Speaker，额外做一次“已知人物同人误拆”检查：逐一比较本集已确认或有可靠依据的人物，尤其是主持人，判断该 Speaker 是否只是同一人的另一组标签。若没有新的身份介绍，而该 Speaker 在访谈后段、彩蛋或片尾继续承担提问、追问、串联或收束关系，且与已知人物的角色连续，应给出 level=inferred 的待确认候选；证据至少包含两条该 Speaker 的实际发言（不要求相邻），并引用一条人物名单或既有关系证据。该建议只供用户确认，不直接发布为人物观点。彩蛋、片尾或节目结束后的时间位置本身不是换人的证据。
如果该 Speaker 有自我介绍、明确的制作/剪辑等独立身份、或节目资料把他列为单独参与者，优先建立独立人物；不要因为他只补问几句就合并到主持人。若“同人误拆”和“额外参与者”都合理，保留候选并写出实质反证；只有没有任何可靠锚点时才返回空 candidates。前面对单次插话不能定主播的限制，不禁止基于多条连续证据提出 inferred 候选。
