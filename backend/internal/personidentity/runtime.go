...
为每个可靠说话人关系返回 speech_bindings：speaker_label、转写 anchor evidence、basis 和 orders。anchor 必须是该标签实际说出的自我介绍、明确第一人称身份引用或可核对的被点名后回应，不能用主持人介绍嘉宾的句子把主持人绑定给嘉宾。随后只列同一标签中能够可靠归属的片段，混标签、插入录音、冒充、冲突区间留空或排除。检查单个片段内部是否混入另一人的简短应答：例如“我补个问题。你说。大家好，我是……”这种片段可以用其中的自我介绍核对身份，但整段同时包含别人发言，必须放入 excluded_orders，不能整段确认；之后清楚由本人连续提问的片段仍可归属。主持人转述属于主持人，不属于被转述者。角色互换和语义判断不足时不能强行整组绑定。
自我身份依据不局限于“我是某人”：讲话人明确描述自己的具名作品、资料库或个人经历，并与节目中该人物的资料相符时，可用first_person_identity。必须引用完整的第一人称所有关系与姓名，例如“我将自己的文章整理成林言文集，交给我的团队”；不能仅因提到林言、阅读林言的书、引用林言的话而绑定给林言。姓名误写仍须source_names的原文关系支持。先核对此锚点，再核对同标签的持续发言，不因缺少公式化自我介绍而把可确认的整组发言全部弃用。
对可能夹杂旁人短应答的片段，核对整个话轮，而非只看主讲者：若中途的肯定、附和或告别无法可靠判断仍是同一个人，则整片段列入excluded_orders，保留原文供一般检索；不能以主要内容属于某人就确认整段。也不要机械地把所有“对”“是的”当成换人，明确属于本人连贯表达的仍保留。
归属必须覆盖可确认的发言，不是只输出几条示例。完整核对一个说话人标签在全文保持同一人时，scope=stable_speaker，服务端将展开该标签的全部片段，excluded_orders 明确排除例外片段；不要再罗列数百个 orders。若多人混用标签或关系不稳定，scope=listed_fragments，orders 列出实际核对的片段，不能扩大到其他片段。缺乏依据时留空，不为覆盖率冒险。
