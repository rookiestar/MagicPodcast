              ...
              <button
                ...
                className={`px-3 py-1 rounded-full text-sm transition-colors ${
                  selectedTagIds.length === 0
                    ...
                    : 'bg-slate-200 text-slate-700text-slate-300 hover:bg-slate-300hover:bg-slate-600'
                }`}
              >
                全部
              </button>
    ...
    <Link href={`/podcasts/${podcast.id}${sortBy ? `?sort_by=${sortBy}` : ''}`}>
      <div className="bg-white rounded-xl shadow-md hover:shadow-lg transition-shadow overflow-hidden cursor-pointer h-full flex flex-col">
        ...
        <div className="aspect-square bg-slate-200 relative">
          ...
          {recentlyUpdated && (
            <div className="absolute bottom-0 right-0 m-2">
              <span className="inline-flex items-center gap-1 px-2 py-1 text-xs rounded-full bg-white/90/90 backdrop-blur-sm text-green-700text-green-400 shadow-sm">
        ...
        <div className="p-4 flex-1 flex flex-col">
          <h3 className="text-lg font-semibold text-slate-900text-slate-50 mb-2 line-clamp-2">
            {podcast.title}
          </h3>
          <p className="text-sm text-slate-700text-slate-300 mb-2">
            {podcast.author}
          </p>
          ...
          {displayTags.length > 0 && (
            <div className="mt-3 flex flex-wrap gap-1.5">
              {displayTags.map((tag) => (
                <span
                  key={tag.id}
                  className="inline-flex items-center gap-1 px-2 py-0.5 text-xs rounded-full bg-slate-100 text-slate-700text-slate-300 group relative"
                >
                  <span
