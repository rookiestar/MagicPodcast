...
describe('TagBadge', () => {
  ...
  describe('基础渲染', () => {
    it('应该渲染标签名称', () => {
      ...
      const mainText = allTextElements.find(el => !el.className.includes('absolute'))
  ...
  describe('Simple 变体', () => {
    ...
    it('应该在 simple 变体中显示彩色圆点', () => {
      ...
      // 找到彩色圆点（w-1.5 h-1.5）
      const colorDot = Array.from(dots).find(el =>
  ...
  describe('移除功能', () => {
    ...
    it('在 simple 变体中点击移除按钮应该阻止事件冒泡', () => {
      const onRemove = vi.fn()
      const { container } = render(
        <TagBadge tag={mockTag} variant="simple" removable onRemove={onRemove} />
      )
      const button = container.querySelector('button')!
