...
export default function PodcastCover({
  ...
  return (
    <div ref={elementRef} className="aspect-square bg-slate-200 relative w-full h-full">
      {/* LQIP: 使用模糊的原图作为占位符 */}
      {coverUrl && !imageError && (
        <img
          src={coverUrl}
      ...
      {coverUrl ? (
        <img
          ref={imgRef}
          alt={title}
          className={`w-full h-full object-cover transition-all duration-500 relative z-20 ${
