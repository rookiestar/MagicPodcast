...
// 获取播客列表
export async function listPodcasts(params?: PodcastFilters): Promise<PodcastListResponse> {
...
// 更新播客备注
export async function updatePodcastNotes(id: number, notes: string): Promise<void> {
...
export async function getPodcastTags(id: number): Promise<any[]> {
  const response = await api.get<ApiResponse<any[]>>(`/api/v1/podcasts/${id}/tags`)
  if (response.data.success && response.data.data) {
    return response.data.data
  }
...
// 为播客添加标签
export async function addTagToPodcast(id: number, tagId: number): Promise<void> {
...
// 移除播客标签
export async function removeTagFromPodcast(id: number, tagId: number): Promise<void> {
