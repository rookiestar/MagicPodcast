...
interface UseWorkflowFormReturn {
  ...
  nextStep: () => void
  prevStep: () => void
  updateField: <K extends keyof WorkflowFormData>(field: K, value: WorkflowFormData[K]) => void
...
export function useWorkflowForm({ workflow, isOpen }: UseWorkflowFormProps): UseWorkflowFormReturn {
  ...
  useEffect(() => { podcastSearchRef.current = podcastSearch }, [podcastSearch])
  useEffect(() => { selectedTagIdsRef.current = formData.selectedTagIds }, [formData.selectedTagIds])
    ...
    if (isOpen) {
      ...
      if (workflow) {
        ...
        setFormData({
          ...
          customCron: CRON_PRESETS.some(p => p.value === workflow.schedule) ? '' : workflow.schedule,
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting && displayedCountRef.current < podcastsRef.current.length) {
          setDisplayedCount(prev => Math.min(prev + 50, podcastsRef.current.length))
        }
      },
      { threshold: 0.1 }
    )
  ...
  // 更新表单字段
  const updateField = useCallback(<K extends keyof WorkflowFormData>(field: K, value: WorkflowFormData[K]) => {
  ...
  const addCustomUrl = useCallback(() => {
    if (newCustomUrl.trim() && !formData.customUrls.includes(newCustomUrl.trim())) {
  ...
  const validateCurrentStep = useCallback((): boolean => {
    switch (step) {
      ...
      case 2:
        // 验证范围配置
        if (formData.scopeType === 'selected' && formData.selectedPodcastIds.length === 0) {
          alert('请至少选择一个节目')
          return false
        }
        if (formData.scopeType === 'custom_urls' && formData.customUrls.length === 0) {
  ...
  }, [step, formData, formData.customCron, formData.schedule, formData.selectedPodcastIds, formData.customUrls])
  ...
  const submit = useCallback(async () => {
    ...
    try {
      ...
      const scopeConfig: ScopeConfig = {
        ...
        podcast_ids: formData.scopeType === 'selected' ? formData.selectedPodcastIds : undefined,
        custom_urls: formData.scopeType === 'custom_urls' ? formData.customUrls : undefined,
        tag_ids: formData.scopeType === 'by_tags' ? formData.selectedTagIds : undefined,
      }
