...
# A non-publisher supervisor cycle must never stop services after a release
# has claimed the maintenance window. The publisher inherits the owner
# context and is allowed through.
if maintenance_info="$(production_maintenance_inspect 2>/dev/null)"; then
  if ! production_maintenance_context_matches; then
    echo -e "${YELLOW}生产发布维护窗口进行中，拒绝非发布方停止服务${NC}" >&2
    exit 1
  fi
fi
